function [Data,UE,A,RTMData]=timestep_sym_init_2nd_gpu(A,E_Init,show,sizes,t,cluster,imagerange,src)
...
nts=2*round(0.9*sizes.Nx/tau/nt); %0109 remember to change it
...
if cluster == 1
UE= zeros(sizes.ex1p+sizes.ey2p,nts*nsrc/2,'gpuArray');  %


elseif cluster == 0

UE = zeros(sizes.NE,nts*nsrc/2);
else
UE = zeros(sizes.NE,nts*nsrc);
...
RTMData(:,:,1) = Dfn'*Fe_curr;
UE(:,indBl(storID))=Fe_curr(IndexCurr,:);
%----------------------------------------------------- 
...
%     disp('Save Data!')
        ...
        Data(:,:,storID)=E_Init'*Fe_curr;
        if or(it/nt <= nts/2,cluster==3)
        UE(:,indBl(storID))=Fe_curr(IndexCurr,:);%alpha cluster
        end
    end
